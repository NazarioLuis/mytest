...
class PeriodoTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.PeriodoTableMap';
     ...
     */
    const COL_ID = 'periodo.id';

    /**
     * the column name for the car_id field
     */
    const COL_CAR_ID = 'periodo.car_id';

    /**
     * the column name for the anio field
     */
    const COL_ANIO = 'periodo.anio';
  ...
  array (
    ...
    {
        return $withPrefix ? PeriodoTableMap::CLASS_DEFAULT : PeriodoTableMap::OM_CLASS;
    }
    ...
    {
        $key = PeriodoTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = PeriodoTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + PeriodoTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = PeriodoTableMap::OM_CLASS;
            /** @var Periodo $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            PeriodoTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = PeriodoTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = PeriodoTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(PeriodoTableMap::COL_ID);
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(PeriodoTableMap::DATABASE_NAME)->getTable(PeriodoTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(PeriodoTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(PeriodoTableMap::TABLE_NAME)) {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            ...
            $criteria->add(PeriodoTableMap::COL_ID, (array) $values, Criteria::IN);
        }

        $query = PeriodoQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            PeriodoTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                PeriodoTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return PeriodoQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Periodo or Criteria object.
     *
     * @param mixed               $criteria Criteria or Periodo object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Periodo object
        }

        if ($criteria->containsKey(PeriodoTableMap::COL_ID) && $criteria->keyContainsValue(PeriodoTableMap::COL_ID) ) {
